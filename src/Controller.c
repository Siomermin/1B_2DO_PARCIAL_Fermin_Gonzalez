...
LinkedList* controller_filterEditorialMinotauro(char* path, LinkedList* pArrayListBooks, int* retorno) // 5
{
	*retorno = ERROR;
...
int controller_map(char* path, LinkedList* pArrayListBooks) // 6
{
	int retorno = ERROR;
    LinkedList* listaAux;
	FILE* pFile;
	...
	pFile = fopen(path, "w");
	if(pArrayListBooks != NULL && pFile != NULL)
	{
		listaAux = ll_map(pArrayListBooks, eLibro_mapPlaneta);
		listaAux = ll_map(pArrayListBooks, eLibro_mapSigloXXIEditores);
	}

	if(listaAux != NULL && parser_SaveFilterListToText(pFile, listaAux) == FUNCIONO)
	{
		retorno = FUNCIONO;
	}
	fclose(pFile);
