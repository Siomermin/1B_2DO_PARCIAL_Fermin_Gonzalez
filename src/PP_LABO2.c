	...
	do
	{
		...
		switch(opcion)
		{
			case 1:
				...
				{
					if(Utn_GetString(path, LEN_AUX, "Ingrese la direccion del archivo: ", MENSAJE_ERROR, REINTENTOS) == FUNCIONO &&
					   controller_loadBooksFromText(path, listaLibros) == FUNCIONO) // BIEN
					{
			...
			case 2:
				...
				{
					if(Utn_GetString(path, LEN_AUX, "Ingrese la direccion del archivo: ", MENSAJE_ERROR, REINTENTOS) == FUNCIONO &&
					   controller_loadEditorialFromText(path, listaEditoriales) == FUNCIONO) // BIEN
					{
			...
			case 3:
				if(flagLoadBooks && flagLoadEditorial) // BIEN CREO
				{
					...
					{
						printf("Los libros se ordenaron satisfactoriamente!\n");
					}
					else
					{
						printf("Hubo un error al ordenar los libros!\n");
					}
				}
				else
				{
			...
			case 5:
				if(flagLoadBooks && flagLoadEditorial) // BIEN CREO
				{
					listaFiltrada = controller_filterEditorialMinotauro(PATH_LISTA_FILTRADA, listaLibros, &retorno);
					if(retorno == FUNCIONO && listaFiltrada != NULL)
					{
						controller_ListBooks(listaFiltrada, listaEditoriales);
				...
				break;
			case 6:
				if(flagLoadBooks && flagLoadEditorial)
				{
					if(controller_map("mapeado.csv", listaLibros) == FUNCIONO)
					{
						printf("La lista se guardo satisfactoriamente!\n");
					}
					else
					{
						printf("Ocurrrio un error al mapear la lista!\n");
					}
				}
				else
				{
