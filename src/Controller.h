...
/// \param pArrayListBooks LinkedList* lista de libros.
/// \return ERROR(-1) si hubo algun error o FUNCIONO(0) si esta todo correcto.
LinkedList* controller_filterEditorialMinotauro(char* path, LinkedList* pArrayListBooks, int* retorno);

/// \fn int controller_map(char*, LinkedList*)
/// \brief Mapea una lista aux llamando a dos funciones criterio que aplican descuentos, una vez aplicados se guarda la lista como texto en un archivo.
/// \param path char* Direccion donde se encuentra el archivo csv.
/// \param pArrayListBooks LinkedList* lista de libros.
/// \return ERROR(-1) si hubo algun error o FUNCIONO(0) si esta todo correcto.
int controller_map(char* path, LinkedList* pArrayListBooks);
