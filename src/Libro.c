...
void* eLibro_mapPlaneta(void* libro)
{
	...
	if(libro != NULL)
	{
		...
		if(eLibro_getIdEditorial(unLibro, &auxIdEditorial) == FUNCIONO)
		{
			if(auxIdEditorial == PLANETA && eLibro_getPrice(unLibro, &auxPrecio) == FUNCIONO) // PLANETA
			{
				if(auxPrecio >= 300)
				{
					eLibro_setPrice(unLibro, auxPrecio*0.80); // 20%
				}
			}
		}
	}
...
void* eLibro_mapSigloXXIEditores(void* libro)
{
	...
	if(libro != NULL)
	{
		...
		if(eLibro_getIdEditorial(unLibro, &auxIdEditorial) == FUNCIONO)
		{
			if(auxIdEditorial == SIGLO_XXI && eLibro_getPrice(unLibro, &auxPrecio) == FUNCIONO)
			{
				if(auxPrecio <= 200)
				{
					eLibro_setPrice(unLibro, auxPrecio*0.90); // 10%
				}
			}
		}
	}
